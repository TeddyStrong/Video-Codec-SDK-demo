...
__global__ inline void YuvChangeKernel(uint8_t *pYuv, int nYuvPitch, int nWidth, int nHeight) {
    ...
    *(YuvUnitx2 *)pSrc = YuvUnitx2 {l0.x / 2, l0.y / 2};
    *(YuvUnitx2 *)(pSrc + nYuvPitch) = YuvUnitx2 {l1.x / 2, l1.y / 2};
}
...
void Nv12ToNv12(uint8_t *dpNv12, int nNv12Pitch, int nWidth, int nHeight) {
    YuvChangeKernel<uchar2>
        <<<dim3((nWidth + 63) / 32 / 2, (nHeight + 3) / 2 / 2), dim3(32, 2)>>>
        (dpNv12, nNv12Pitch, nWidth, nHeight);
}
